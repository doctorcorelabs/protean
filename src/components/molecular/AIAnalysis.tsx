...
import { Brain, TrendingUp, CheckCircle, BarChart3, Target, Database, Search } from 'lucide-react'
...
interface AnalysisResult {
  ...
  mutationalInsights?: {
    criticalResidues?: string[]
    mutationSensitivity?: string
    conservation?: string
  }
  experimentalDetails?: {
    resolution?: string
...
Format your response as valid JSON with the following structure:
{
  ...
  const tabs = [
    ...
    { id: 'details', label: 'Molecular Details', icon: Database },
            ...
            <div className="lg:col-span-2">
              {result && (
                <GlassCard>
                  ...
                  {/* Tab Navigation */}
                  <div className="flex space-x-1 mb-6 bg-gray-100 rounded-lg p-1 overflow-x-auto max-w-full scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
                    {tabs.map((tab) => {
                      const Icon = tab.icon
                      return (
                        <button
                  ...
                  <div className="space-y-6 max-w-full overflow-x-auto">
                    {selectedTab === 'details' && (
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">Molecular Details</h4>
                        {/* Domain Architecture */}
                        <div className="mb-6">
                          <h5 className="font-semibold mb-2">Domain Architecture</h5>
                          {result?.domainArchitecture ? (
                            <>
                              <div className="mb-2"><b>Domains:</b> {result.domainArchitecture.domains?.length ? result.domainArchitecture.domains.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>Motifs:</b> {result.domainArchitecture.motifs?.length ? result.domainArchitecture.motifs.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>Secondary Structure:</b> {result.domainArchitecture.secondaryStructure?.length ? result.domainArchitecture.secondaryStructure.join(', ') : 'N/A'}</div>
                            </>
                          ) : <div className="text-gray-500">No domain architecture data available.</div>}
                        </div>
                        {/* Mutational Insights */}
                        <div className="mb-6">
                          <h5 className="font-semibold mb-2">Mutational Insights</h5>
                          {result?.mutationalInsights ? (
                            <>
                              <div className="mb-2"><b>Critical Residues:</b> {result.mutationalInsights.criticalResidues?.length ? result.mutationalInsights.criticalResidues.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>Mutation Sensitivity:</b> {result.mutationalInsights.mutationSensitivity || 'N/A'}</div>
                              <div className="mb-2"><b>Conservation:</b> {result.mutationalInsights.conservation || 'N/A'}</div>
                            </>
                          ) : <div className="text-gray-500">No mutational insights data available.</div>}
                        </div>
                        {/* Experimental Details */}
                        <div className="mb-6">
                          <h5 className="font-semibold mb-2">Experimental Details</h5>
                          {result?.experimentalDetails ? (
                            <>
                              <div className="mb-2"><b>Resolution:</b> {result.experimentalDetails.resolution || 'N/A'}</div>
                              <div className="mb-2"><b>pH:</b> {result.experimentalDetails.pH || 'N/A'}</div>
                              <div className="mb-2"><b>Temperature:</b> {result.experimentalDetails.temperature || 'N/A'}</div>
                              <div className="mb-2"><b>Method:</b> {result.experimentalDetails.method || 'N/A'}</div>
                            </>
                          ) : <div className="text-gray-500">No experimental details available.</div>}
                        </div>
                        {/* Chemical Components */}
                        <div className="mb-6">
                          <h5 className="font-semibold mb-2">Chemical Components</h5>
                          {result?.chemicalComponents?.length ? (
                            <ul className="list-disc ml-6">
                              {result.chemicalComponents.map((comp, idx) => (
                                <li key={idx}><b>{comp.name}</b> ({comp.type}), Formula: {comp.formula}, MW: {comp.molecularWeight}</li>
                              ))}
                            </ul>
                          ) : <div className="text-gray-500">No chemical components data available.</div>}
                        </div>
                        {/* Cross-References */}
                        <div className="mb-6">
                          <h5 className="font-semibold mb-2">Cross-References</h5>
                          {result?.crossReferences ? (
                            <>
                              <div className="mb-2"><b>UniProt:</b> {result.crossReferences.uniprot?.length ? result.crossReferences.uniprot.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>GenBank:</b> {result.crossReferences.genbank?.length ? result.crossReferences.genbank.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>DrugBank:</b> {result.crossReferences.drugbank?.length ? result.crossReferences.drugbank.join(', ') : 'N/A'}</div>
                              <div className="mb-2"><b>Other:</b> {result.crossReferences.other?.length ? result.crossReferences.other.join(', ') : 'N/A'}</div>
                            </>
                          ) : <div className="text-gray-500">No cross-references data available.</div>}
                        </div>
                      </div>
                    )}
                              ...
                              <div key={index} className="border border-gray-200 rounded-lg p-4">
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-1 mt-2">
                                  <div
                                    ...
                                    style={{ width: `${Math.min(interaction.strength * 100, 100)}%` }}
                                  ></div>
                                </div>
                              </div>
                            ))}
